...
interface GalleryEditModalProps {
  ...
  onSave: (imageData: { title: string; description: string; website: string; links: Array<{ label: string; url: string }> }) => Promise<void>;
}

/**
 * Modal para editar imágenes de la galería
 * 
 * Permite editar los datos de una imagen de la galería
...
const GalleryEditModal: React.FC<GalleryEditModalProps> = ({
  ...
  // Cargar datos de la imagen cuando se abre el modal
  useEffect(() => {
    if (isOpen && image) {
      setFormData({
        title: image.title || '',
        ...
        links: [{ label: 'Sitio Web', url: image.website || '' }]
      });
    }
  }, [isOpen, image]);
  ...
  const handleSave = async () => {
    if (!formData.title.trim()) {
      showNotification('error', 'Error de validación', 'El nombre de la universidad es obligatorio');
      return;
    }
    ...
    try {
      ...
      showNotification('success', 'Éxito', 'Imagen actualizada exitosamente');
      onClose();
    } catch {
      showNotification('error', 'Error', 'Error al actualizar la imagen');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        >
          ...
          <div style={{ marginBottom: `${scale(16)}px` }}>
            ...
            <textarea
              ...
              placeholder="Ingrese una descripción de la universidad"
            />
          </div>
