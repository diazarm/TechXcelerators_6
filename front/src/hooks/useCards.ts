...
type PageType = 'dashboard' | 'gobernanza' | 'alianza' | 'gestion';
...
export const useCards = ({ pageType, onEditClick, onDeleteClick }: UseCardsProps): UseCardsReturn => {
  ...
  const cards = useMemo(() => {
    ...
    // Mostrar acciones (iconos de la derecha) solo para admin o director en las páginas Alianza y Gestión
    if (pageType === 'alianza' || pageType === 'gestion') {
