...
export const updateResource = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        res.status(200).json({success: true, message: "Recurso actualizado correctamente", data: resource});
    } catch (error) {
        next(error);
    }
};
...
export const restoreResource = async (req: Request, res: Response, next: NextFunction) => {
  try {
